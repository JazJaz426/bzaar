...
// import { getItemDetails, getSellerProfile, claimItem, logInteraction, getClaimList } from '../utils/api';
...
//         console.log(claimListData)
//     };
//     const isClaimedByCurrentUser = claimList.includes(item.id);
...
const ItemDetail = () => {
    ...
    const handleClaimItem = async () => {
        if (item && id) {
            const operation = isClaimedByUser ? 'del' : 'add';
            const newStatus = isClaimedByUser ? 'available' : 'claimed';
    ...
    console.log("true or false: ", item.status == 'available' || isClaimedByUser )
